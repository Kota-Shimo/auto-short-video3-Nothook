...
# ---------- 1ブロック（1段）を作る ----------
def _make_block(text: str, font_size: int, cjk_width: int) -> tuple[TextClip, ColorClip]:
    body = _prep_line(text, font_size=font_size, cjk_width=cjk_width)
    clip = TextClip(
        text=body,
        font=pick_font(body),
        font_size=font_size,
...
def build_video(
    ...
    # 行ごとの設定テーブル（rowsに応じて使用）
    # index: 0=上段, 1=ふりがな(あれば), 2=下段(あれば)
    FONT_SIZES   = [FS_TOP,  FS_FURI,  FS_BOT]
    CJK_WIDTHS   = [CJK_WIDTH_TOP, CJK_WIDTH_FURI, CJK_WIDTH_BOT]
    LINE_GAPS    = [LINE_GAP_TOP_TO_FURI, LINE_GAP_FURI_TO_BOT]  # between 0→1, 1→2
    # 2段の場合は従来値を踏襲
    if rows == 2:
        LINE_GAPS[0] = LINE_GAP_DEFAULT

    for item in lines:
        # item: (speaker, row1[, row2[, row3]], duration)
        ...
        for i in range(rows):
            ...
            # ラベルは上段のみ対象（Nなら非表示）
            if i == 0:
                top_txt = text_i if (is_n and hide_n_label) else f"{speaker}: {text_i}"
                clip_i, bg_i = _make_block(
                    text=top_txt,
            ...
            if i == 0:
                ...
                block_h = (y_i - current_y) + bg_i.h  # 先頭段yからの増分
            current_y = POS_Y  # 起点は常にPOS_Y
            total_block_h = (elem[-1].pos(0)[1] + bg_i.h + PAD_Y) - POS_Y  # 先頭起点からの合計高さ
